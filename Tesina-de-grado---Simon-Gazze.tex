...
convertidos por cada equipo a lo largo del partido, obteniéndose un
marcador reconstruido que reemplazó al obtenido originalmente en el
proceso de \emph{scrapping}.
...
En particular, se observaron situaciones en las que dos acciones de
...
menor será cada una de las respectivas razones de odds mencionadas.

Como el objetivo de esta tesina está centrado en obtener una medida
única que resuma la contribución de un jugador en el desempeño ofensivo
y defensivo de su equipo, estos coeficientes no serán interpretados de
manera individual, sino que servirán como base para la construcción de
una métrica unificada.
...
A diferencia del modelo de regresión multinomial, el cambio en la
...
En el modelo de regresión lineal múltiple, se van a estimar los
parámetros por mínimos cuadrados ponderados, siendo la función a
minimizar:
...
\begin{equation}
\hat{\beta_k} = \frac{\sum_{i=1}^n x_{ik}^*y_{i}}{\sum_{i=1}^n w_i(x_{ik}^*)^2}
\end{equation},
...
identificarse en el cuadro X. Como resultado, el modelo dispone de poca
...
figura 11, observándose jugadores que participaron en apenas una o dos
...
\begin{equation}
L^*_m(\boldsymbol{\beta},\boldsymbol{y})=\sum_{i=1}^nw_i\sum_{j=1}^cy_{ij}log(\pi_{ij}) + \lambda \sum_{k=1}^p|\beta_k|=L_m(\boldsymbol{\beta},\boldsymbol{y}) + \lambda \sum_{k=1}^p|\beta_k|
\end{equation}
...
modelos planteados en base a medidas de error y comprobación de
supuestos (o En primera instancia, se evalúa la bondad de ajuste de los
distintos modelos mediante medidas de error predictivo y procedimientos
de validación basados en simulación.).quizas sea una primer apartado de
análisis de supuestos/comprobación de supuestos/ comprobación de
utilidad del modelo o algo así para mostrar que el modelo de regresión
...
metodología para medir la consistencia de nuestros modelos, mientras que
para medir la validez del mismo se recurre a criterios de validación
...
normal}\label{diagnuxf3stico-del-modelo-normal}
...
Además de evaluar el desempeño predictivo mediante el RMSE, se realizó
una validación adicional basada en simulaciones del modelo multinomial.
El objetivo de este procedimiento es verificar si el modelo reproduce
adecuadamente la distribución empírica de puntos por posesión.
