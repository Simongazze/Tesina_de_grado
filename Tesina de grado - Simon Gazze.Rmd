...
\begin{equation}
L^*_m(\boldsymbol{\beta},\boldsymbol{y})=\sum_{i=1}^n\sum_{j=1}^cy_{ij}log(\pi_{ij}) + \lambda \sum_{k=1}^p[\alpha\sum_{k=1}^p|\beta_k|+\frac{(1-\alpha)}{2}\sum_{k=1}^p\beta_k^2]=L_m(\boldsymbol{\beta},\boldsymbol{y}) + \lambda \sum_{k=1}^p[\alpha\sum_{k=1}^p|\beta_k|+\frac{(1-\alpha)}{2}\sum_{k=1}^p\beta_k^2]
\end{equation}
...
### Análisis del *plus-minus*

Dentro del universo de medidas que buscan cuantificar el aporte realizado por los jugadores en relación al desempeño de sus equipos, la métrica *plus-minus* (+/-) brinda un indicio de cuan eficiente fue el jugador en el tiempo que permaneció en cancha, calculando la diferencia de puntos realizados por el equipo propio en comparación con los puntos del rival dentro de ese intervalo de tiempo.

Los lideres en +/- para la temporada 2024/2025 de la LNB fueron los siguientes:

```{r, fig.cap="Mejores y peores *plus-minus* de la temporada", out.width="80%", fig.align="center"}

tabla_plus_minus = readRDS("tabla_plus_minus.rds")
...
El cuadro 5 deja en evidencia que los jugadores con mejor +/- pertenecen a los equipos que mejores resultados obtuvieron a lo largo del año. Como se ve en el Cuadro 6, dentro de los 11 mejores jugadores en base a esta métrica, 9 pertenecían a uno de los 4 primeros de la temporada (Boca Juniors, Oberá, Instituto y Quimsa), mientras que los casos de Marcos Chacón Tirado y Diego Rafael Figueredo resultaron casos atípicos pertenecientes a equipos de mitad de tabla.
...
tabla_gt = readRDS("tabla_posiciones.rds")
