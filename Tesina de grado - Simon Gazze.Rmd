...
El valor de la categoría "3 o más" corresponde al promedio empírico de puntos en esta categoría superior a partir de la distribución observada:
...
$$PM^{100}_j = \frac{PM_j \cdot 100}{N_j}$$
donde $PM_j$ representa el *plus-minus* acumulado del jugador j y $N_j$ la cantidad de posesiones en las que participó.
...
Valores positivos indican que el jugador registró un +/- por 100 posesiones superior a la mediana de sus compañeros, mientras que los valores negativos señalan un desempeño inferior en términos relativos dentro de su equipo.
...
tabla_penalizacion <- data.frame(
  Penalización = c("Ridge", "Lasso", "Elastic Net"),
  `$\\alpha$` = c("0", "1", "0,005"),
  `$\\lambda$` = c("0.06258", "0.00085", "0.04261"),
  `CME CV` = c("1.405088", "1.405401", "1,405031"),
  `Coeficientes activos` = c("458", "98", "331")
)
...
tabla_penalizacion <- data.frame(
  ...
  `Deviance CV` = c("2.195519", "2.196606", "2.195772"),
  `Predictores activos` = c("458", "199", "457")
)
...
Mientras que para *LASSO* se observa que, al igual que en los modelos normales, la distribución de las métricas es más dispera, con colas más pronunciadas. Debido en gran medida a la cantidad de coeficientes reducidos a 0 en la regularización.

### wEPTS

Una vez obtenidos los $EPTS$ se calcula su versión ponderada según la proporción de posesiones de su equipo en las que participó cada jugador. Este índice actua como un *shrinkage* sobre el EPTS penalizando a los jugadores con menos tiempo de juego, contrayendo sus EPTS hacia los puntos esperados del grupo de referencia. En caso de que el jugador si participara en todas las posesiones de su equipo, el $wEPTS$ sería igual al $EPTS$.
...
Al considerar las métricas ofensivas, se observa que las mayores reducciones absolutas no se concentran necesariamente entre quienes presentan muy baja participación durante la temporada. Cuatro de los cinco jugadores mejor ubicados en el ranking ofensivo de *EPTS* se encontraron entre los más afectados por la ponderación, a pesar de haber participado aproximadamente en el 40%–45% de las posesiones de sus equipos. Como consecuencia, registraron caídas relevantes en el ranking:
...
Al analizar las métricas defensivas también se observa un comportamiento similar al mencionado.

```{r, echo=FALSE, fig.cap="Comparación de distribuciones: EPTS vs wEPTS", out.width="100%", fig.align="center"}
knitr::include_graphics("img/grafico_defensivo_wepts.pdf")
```

Donde destaca `SANDERS, NAKIE GERALD`, jugador que presentó la mayor reducción en la métrica entre todos los predictores (-0,02215). Pasando del 9no puesto defensivo en el ranking EPTS al 39° en el ranking wEPTS. 

Entre los 5 jugadores con mayor penalización en su métrica defensiva encontramos, al igual que sucedió con la métrica defensiva, al jugador `CHACON TIRADO, MARCOS`, el cual pasó del 7mo puesto defensivo al 13vo en el nuevo ranking. Identificandose como uno de los jugadores más afectados por esta nueva métrica ponderada.
...
Por lo que la distancia respecto de $EPTS_0$ se reduce proporcionalmente según $W_k$. De esta manera, quienes tienen menor participación siempre van a presentar una contracción **proporcional** mayor. Sin embargo, el cambio absoluto es:

$$|wEPTS_k-EPTS_k|=(1-W_k)|EPTS_k-EPTS_0|$$

Por eso, un jugador con una participación intermedia (como la gran mayoría de casos que se vieron anteriormente) puede ser uno de los más afectado en términos absolutos si su EPTS original es muy alto o muy bajo, y por lo tanto muy lejano a $EPTS_0$.

De esta manera, wEPTS no solo estabiliza las estimaciones de jugadores con escasa presencia, sino que modifica la interpretación del ranking al incorporar la proporción de posesiones disputadas durante la temporada.

En conclusión, ambas medidas parecen abordar un problema diferente, wEPTS representa una **medida de contribución de los jugadores a lo largo de toda la temporada**, mientras que EPTS describe principalmente el **rendimiento esperado cuando el jugador se encuentra en cancha**.
