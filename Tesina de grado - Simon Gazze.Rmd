...
#### Interpretación de los coeficientesREVISAR
...
En los modelos de regresión múltiple, la multicolinealidad hace referencia a la existencia de dependencias lineales o casi lineales entre las columnas de la matriz de diseño. Cuando estas dependencias son exactas, la matriz $X'X$ resulta singular y los parámetros no pueden estimarse mediante mínimos cuadrados ordinarios. Cuando las dependencias son aproximadas, las estimaciones pueden calculares, pero presentando variancias grandes, dando lugar a coeficientes muy sensibles a cambios relativamente pequeños en los datos.
...
Particularmente, en el ámbito del fútbol, Lars Magnus Hvattum, quien ha desarrollado una extensa línea de investigación sobre métricas *plus-minus* para la evaluación de jugadores, plantea distintas propuestas metodológicas para la comparación de modelos, las cuales no parecen haber sido implementada en básquetbol hasta la fecha. En el artículo *Modelling the financial contribution of soccer players to their clubs* (Sæbø & Hvattum, 2019) y en videos de su canal de YouTube [Football Player Ratings](https://www.youtube.com/@footballplayerratings) se definen dos criterios para medir los distintos sistemas de rankings. Esta metodología es resplicada para medir la consistencia de los modelos realizados, mientras que, para evaluar su validez, se recurre a criterios de validación externa, tomando como referencia la propuesta de Damoulaki et al. (2025) pero adaptándola a la información disponible en la Liga Nacional de Básquet Argentina.
...
En cambio, el modelo logístico multinomial considera a los puntos por posesión como variable categórica, permitiendo estimar las probabilidades $\hat{\pi}_{ic}$ para cada categoría de anotación. Para obtener una predicción puntual en la escala original de puntos, se utiliza una ponderación análoga a la definida en la Sección 4.2.1 para la construcción del EPTS, pero calculando las probabilidades sobre el conjunto de jugadores presentes en la posesión $i$:
...
poner las comparación entre epts y wepts, mientras que epts podría reflejear el impacto del jugador en el tiempo que estuvo en cancha, el wepts al penalizar a los jugadores que jugaron poco, da como una medida de quien fue mejor en toda la temporada. son medidas que a priori se comparan y ambas buscan medir cual es el mejor jugador, pero pueden ser complementarias, mientras que epts puede servir a la hora de hacer scouting y ver que jugador rindió mejor en la cantidad de posesiones que le tocó jugar, wepts puede servir para definir la premiación de los jugadores de la temporada, dando más valor a los que más jugadoron, tomando a la temporada como un todo y el aporte que hacen los jugadores a ese todo.
