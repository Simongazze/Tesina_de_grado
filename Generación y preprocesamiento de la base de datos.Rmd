...
header-includes:
  - \usepackage{caption}
  - \usepackage{xcolor}
  - \captionsetup{font={bf, small}, labelfont={bf}, justification=centering, position=top, singlelinecheck=false}
  - \captionsetup[table]{position=top, skip=5pt}
  - \captionsetup[figure]{position=above, skip=5pt}
---
...
```{r, echo=FALSE, fig.cap="Fixture dentro de la página web de la LNB.", out.width="70%", fig.align="center", fig.pos="H"}
...
Una vez hecho esto, se identificaron todas las etiquetas HTML que contenían enlaces a las estadísticas detalladas de cada partido, reconociendo un patrón común en las URLs asociadas a los encuentros individuales, correspondiente a la ruta utilizada para las páginas de estadísticas de cada partido (estadisticascabb.gesdeportiva.es/partido/). Estos enlaces fueron almacenados en una estructura de datos y posteriormente volcados a un archivo de texto (.txt). Dicho archivo constituyó el insumo básico para el proceso posterior de scraping masivo, sobre los cuales se iterará para obtener así la información correspondiente a cada una de las acciones registradas en cada partido.
...
```{r, echo=FALSE, fig.cap="Ejemplo de secuencia de acciones con sustitución", out.width="40%", fig.align="center"}
